<div>
  ...
  <div class="mb-8 flex items-center gap-4">
    <a href="{{ route('requisitions.show', $prId) }}"
    ...
    <div>
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Requisition</h1>
      <p class="text-sm text-gray-500 dark:text-neutral-400 mt-1">{{ $this->pr->reference_no }} &middot; Changes will replace the current draft</p>
    </div>
  </div>
  ...
  <form wire:submit="save" class="space-y-6 max-w-4xl">
    ...
    <div class="bg-white dark:bg-neutral-800 rounded-xl border border-gray-200 dark:border-neutral-700 p-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        {{-- Type (read-only) --}}
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-neutral-300 mb-1.5">Type</label>
          <div class="flex items-center gap-2 px-3 py-2 rounded-lg border border-gray-200 dark:border-neutral-600 bg-gray-50 dark:bg-neutral-700/50 text-sm text-gray-700 dark:text-neutral-300">
            <ph-lock weight="fill" class="text-gray-400 dark:text-neutral-500 text-sm flex-shrink-0"></ph-lock>
            <span class="capitalize">{{ $type === 'product' ? 'Product / Goods' : 'Service' }}</span>
          </div>
          <p class="text-xs text-gray-400 dark:text-neutral-500 mt-1">Type cannot be changed after submission.</p>
        </div>
    ...
    <div class="flex items-center justify-between">
      ...
      <button type="submit"
        ...
        <span wire:loading.remove wire:target="save">
          <ph-floppy-disk weight="fill" class="text-base"></ph-floppy-disk>
        </span>
